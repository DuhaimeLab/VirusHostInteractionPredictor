...
def test_GeneSet_init():
    ...
    # test 1 - test GeneSet object creation generates expected class attributes
    test_GeneSet = GeneSet("tests/datatests/test_annotated_genes.ffn")
    assert len(test_GeneSet.genes) == 3
    assert test_GeneSet.gene_ids == ["ABDEAL_00005", "ABDEAL_00010", "ABDEAL_00015"]
    assert test_GeneSet.gene_products == [
